...
from trainer import cv_fit, data_utils
from trainer.callbacks import TensorBoardCallback
...
class Model:
    ...
    def train(self, config):
        ...
        train = data_utils.load_dataset(config.data_input_path + "/train.csv")
        train_len = len(train)
        test = data_utils.load_dataset(config.data_input_path + "/test.csv")
        ...
        Y_train = pd.to_numeric(train["Survived"].astype(int), downcast="integer")
        ...
        model_file_name = f"titanic_classifier_{date.today()}.joblib"
        stats_df = {
        ...
        logger.info("Store training job outputs to local disk before uploading.")
        #bst.get_booster().save_model(model_file_name).
        ...
        try:
            if config.trainer_output_path.startswith("gs://"):
                ...
                for file_name in gfile.glob("./tb_logs/*"):
                    gfile.copy(
                    file_name
                    , config.trainer_output_path + "/tensorboard/titanic_classifier", overwrite=True
                )
